...
| High | The infrastructure module catalog contains stale resource names and interfaces. | **Resolved for `infra/MODULES.md`:** its resource entries now match the current Terraform declarations, and the hybrid POC modules are documented. Azure/GCP approval README reconciliation remains a separate follow-up. | Reconcile the remaining Azure/GCP approval README claims against provider schemas. |
