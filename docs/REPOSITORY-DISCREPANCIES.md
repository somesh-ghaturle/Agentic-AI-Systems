...
| Medium | The hybrid POC resolved cross-cloud endpoints and discarded them. | `infra/terraform-hybrid/modules/aws-orchestrator` declared `tool_endpoint` and `state_endpoint` and the dev root passed both in, but no resource read either, so `ARCHITECTURE.md`'s claim that the state machine "receives read-only tool endpoint configuration" was not true of the applied configuration. The endpoints now go into the Step Functions definition the root builds, and the module drops the two variables. | Prefer wiring a value into a resource over declaring it; a variable no resource reads is a claim the plan does not make. |
...
- Every Terraform module and env root was scanned for variables no resource reads (58
  directories). The hybrid `aws-orchestrator` module was the only instance and is fixed;
  the other four trees declare nothing they do not use.
- Every tracked Markdown table was scanned for rows whose cell count differs from the rest
  of their table, the failure an unescaped `\|` inside a cell produces. One row in this
  report was truncating on render and is fixed; no other table in the repository has it.
