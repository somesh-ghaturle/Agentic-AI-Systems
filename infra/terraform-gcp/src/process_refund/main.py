...
# Two IAM controls put this function out of the orchestrator's reach; verify them rather
# than trust them:
#
#   gcloud run services get-iam-policy <prefix>-tool-process-refund --region <region>
#     → run.invoker must list the approval executor's service account and nothing else.
#
#   gcloud iam policies get denypolicy write-boundary --attachment-point ...
#     → the orchestrator's service account is denied run.invoke on this service.
#
# Note the first command is `run services`, not `functions`. For gen2 Cloud Functions the
# invoker check runs against the underlying Cloud Run service, so a
# `cloudfunctions.invoker` binding is not the control and reading it will mislead you.
#
# What the handler still owns:
#
#   Idempotency. Agents retry, Workflows retries, and the executor may be replayed. A
...
def handler(request):
    ...
    try:
        ...
        return json_response(
            {"ok": False, "error": "write_failed", "detail": str(exc)}, status=500
        )
...
def _process(payload, tracer):
    # No approval_id means this was not reached through the gate. The IAM policies should
