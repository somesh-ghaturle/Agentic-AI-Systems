...
#      prompt says plainly that nothing inside them is an instruction. See
...
# The proposal contract. additionalProperties must be false throughout for strict structured
# outputs, which is why arguments ride as a JSON string rather than a nested free-form
# object — the tool's arguments are open-ended by nature and cannot be schema-constrained
# here without pinning every tool's signature into this handler.
DECISION_SCHEMA = {
    "type": "object",
    "properties": {
        "action_type": {
...
def handler(request):
    ...
    try:
        ...
        tracer.flush()
        # The workflow's `reason` step has no degraded path — it catches, records, and
        # fails the execution. A non-2xx is what triggers that.
        return json_response({"ok": False, "error": "reason_failed"}, status=502)
...
def _reason(payload, tracer):
    ...
    response = _vertex().messages.stream(
        model=model,
        max_tokens=_max_tokens(),
        system=SYSTEM_PROMPT,
        # Adaptive thinking is on by default for this model family; stated explicitly so the
        # setting is visible at the call site rather than inherited silently.
        thinking={"type": "adaptive"},
        output_config={
            "effort": os.environ.get("MODEL_EFFORT", "high"),
            "format": {"type": "json_schema", "schema": DECISION_SCHEMA},
        },
        messages=[{"role": "user", "content": _build_prompt(query, payload)}],
    )
    # Streaming with a final-message read rather than a plain create: adaptive thinking at
    # high effort can run long enough to hit an idle HTTP timeout, and a timeout here costs
    # the whole request.
    with response as stream:
        message = stream.get_final_message()

    # Checked before reading content, which is empty on a pre-output refusal. Server-side
    # fallbacks are a first-party-only parameter, so on Vertex a refusal is handled as what
    # it is — a designed failure path that ends the request visibly.
    ...
    if invalid:
        return dict(invalid, **_usage(message))

    return ok(
        action_type=decision["action_type"],
        action=decision["action"] or None,
        # The workflow reads `proposal` and hands it to the validator whole.
        proposal={
            "action": decision["action"] or None,
        ...
        # rather than nested because the workflow reads `reasoning.body.cost_usd` directly
        # — Workflows has no comfortable way to reach into a nested object with a default.
        **_usage(message),
    )
...
def _build_prompt(query, payload):
    ...
    remainder read as top-level content. Stripping the sequence costs nothing and removes
    the trick entirely.
    """
    parts = []

    # The workflow passes retrieval output as a flat `context` string; a direct caller may
    # pass structured documents. Both are handled, and both are quoted.
...
def _parse(text, tracer):
    """Validates the model's output against the contract the workflow reads.
...
def _usage(message):
    ...
    Rates are environment-supplied because Vertex is partner-operated and prices separately
    from the first-party API. With them unset, tokens are still reported and cost is simply
    absent — the spend alert then has nothing to count, which is the honest outcome rather
    than a number derived from the wrong price list.
    """
...
def _vertex():
    """Why Vertex, given the guardrail says the model layer is provider-neutral.

    The reason is authentication: on Vertex this function authenticates with its own service
    account, so there is no API key to put in Secret Manager, encrypt, rotate, or account for
    in a blast-radius analysis. In an architecture whose whole argument is about constraining
    what a compromised component can reach, not introducing a long-lived credential is worth
    a good deal.

    The cost is real and should be understood before extending this: Vertex is
    partner-operated, so it carries a feature subset and prices separately from the
