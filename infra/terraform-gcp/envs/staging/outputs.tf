...
output "trace_archive_locked" {
  description = "Whether the archive's retention policy is locked. Should be FALSE here — the inverse of prod. A locked policy commits the bucket for the length of its retention window and cannot be undone by anyone, which would make this environment impossible to tear down and rebuild. True here means someone locked it by hand; the bucket is now permanent."
