...
variable "allowed_ip_list" {
  description = "CIDRs permitted to authenticate as the service users. Enforced in staging so a missing CIDR fails here rather than in prod."
  type        = list(string)
  default     = []
}

variable "task_started" {
  description = "Whether the approval sweeper runs. True in staging, so the approval path is exercised on prod's cadence."
  type        = bool
  default     = true
}
