variable "project" {
  ...
  validation {
    condition     = can(regex("^[a-z0-9-]{1,11}$", var.project))
    error_message = "project must be lowercase alphanumeric/hyphens and at most 11 chars, so \"<project>-dev-approvals\" stays within Azure's 24-char Cosmos and storage account name limits."
  }
}
