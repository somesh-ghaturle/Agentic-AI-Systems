...
output "log_analytics_workspace_id" {
  description = "Workspace diagnostic settings should point at. Unlike prod's, this one is not the target for envs/tenant — Entra audit records are tenant-wide and belong in the workspace someone actually watches, which is prod's."
  value       = module.observability.log_analytics_workspace_id
}

output "action_group_id" {
  description = "Action group every alert notifies. Point its receivers at a staging channel — rehearsal alerts that page the prod on-call rotation train people to ignore the alert."
