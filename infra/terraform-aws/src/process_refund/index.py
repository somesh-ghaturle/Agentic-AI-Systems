...
# The resource policy on this function admits exactly one principal, the approval
# executor, so the orchestrator cannot reach it at all. Verify that rather than trust it:
#
#   aws lambda get-policy --function-name <project>-<env>-tool-process_refund
#
# What the handler still owns:
#
#   Idempotency. Agents retry, Step Functions retries, and the executor may be replayed.
#   A refund that runs twice is a real loss, so the idempotency key travels to the payment
...
def handler(event, context=None):
    tracer = tracer_for(event, step="process_refund")
...
def _process(event, tracer):
    # No approval_id means this was not reached through the gate. The IAM policy should
    # have made that impossible; refusing here makes it impossible twice.
    identity, missing = require(event, "approval_id", "idempotency_key")
    if missing:
        return missing
...
def _submit_refund(order_id, amount_cents, currency, reason, idempotency_key):
    ...
    Two requirements on whatever goes here, both non-negotiable:
      ...
      1. Pass `idempotency_key` to the provider as its idempotency header. Every major
         provider supports one. Retrying with the same key must return the original
         result, not create a second refund.
